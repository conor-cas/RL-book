...
class OptimalExerciseBI:
            ...
            def step(
                ...
                def sr_sampler_func(
                    ...
                    if exer:
                        ...
                        next_price: float = price.state + np.random.normal(0, 1)
    ...
    def get_states_distribution(
        ...
        mean: float = spot_mean2 / np.sqrt(spot_var + spot_mean2)
        stdev: float = np.sqrt(spot_var / spot_mean2 + 1)

        time: float = t * self.expiry / self.num_steps

        def states_sampler_func() -> NonTerminal[float]:
            start: float = np.random.normal(self.spot_price, np.sqrt(spot_var))
            price = np.random.normal(
                start + (self.rate - self.vol * self.vol / 2) * time,
    ...
    def backward_induction_vf_and_pi(
        ...
        mdp_f0_mu_triples: Sequence[Tuple[
            ...
            FunctionApprox[NonTerminal[float]],  # for optimal value function approx
            SampledDistribution[NonTerminal[float]]  # sample the source states at each time step
        ]] = [(
            self.get_mdp(t=i),
    ...
    def put_option_exercise_boundary(
            self,
            opt_vfs: Sequence[FunctionApprox[NonTerminal[float]]],  # seq of optimal policies
            strike: float
    ) -> Sequence[float]:
        ret: List[float] = []
        # prices: np.ndarray = np.arange(0., strike + 0.1, 0.1)
        dt: float = self.dt()
        for vf in opt_vfs[:-1]:  # American Put Option
            ex_points = [j for j in range(vf + 1)
                         if opt_vfs[vf].action_for[j] and
                         self.payoff(vf * dt, self.state_price(vf, j)) > 0]
            if len(ex_points) > 0:
                boundary_pt = max(ex_points)
                ret.append(self.state_price(vf, boundary_pt))
